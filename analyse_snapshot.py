    ...
    for ji in jumbos:
        if hasattr(ji, "child1"):
            ...
            if e<1:
                ...
                if not hasattr(nn, "planets"):
                    nn.planets = Particles()
            ...
            print(f"N(SJ)= {len(sma)}, N(sp)={len(singles)}")
    #print("sma=", sma)
    #print(ecc)
...
def find_nearest_star(planets, stars):
    ...
    for pi in planets:
        ...
        print("n=", len(r), len(planets), len(stars))
        r, s = (list(t) for t in zip(*sorted(zip(r, stars))))
        print(r[0].in_(units.au))
...
if __name__=="__main__":
    ...
    r = numpy.zeros(len(nearest_star))
    for ji in range(len(nearest_star)):
        r.append((nearest_star[ji].position-bound_jumbos[ji].position).length())
    ...
    if len(bound_jumbos)>0:
        ...
        plt.scatter(aj.value_in(units.au), ep, c='b')
    else:
