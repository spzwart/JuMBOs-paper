...
from src.make_jumbos import make_outer_planetary_systems, make_isolated_jumbos
...
class RunCode(object):
    def __init__(self, mmin, mmax, Nstars, Njumbos, Nff,
        ...
        self.Acorr = 0 | (units.m**2 * units.kg * units.s**-1)
        self.Ecorr = 0 | units.J
        ...
        for parti_ in range(len(particle_set)):
            if particle_set[parti_].Nej == 0:
                ...
                if traj > 0 and rel_pos[parti_].length() > cluster_lim:
                    ...
                    if KE > abs(PE):
                    # print("...Detection: Cluster Ejection...")
                        ...
                        self.event_type.append("Ejection")
                            ...
                            #Ai = self.ang_cons(particle_set)
                            #Ei = bodies.potential_energy() + bodies.kinetic_energy()
                            #bodies.remove_particle(particle_set[parti_])
                            #bodies.synchronize_to(gravity.particles)
                            #Af = self.ang_cons(gravity.particles)
                        Ef = bodies.potential_energy() + bodies.kinetic_energy()
                        ejec = True

                            #self.Acorr = (Af - Ai)
                        self.Ecorr = Ef-Ef#(Ef - Ei)
    ...
    def run_cluster(self, bodies, t_end, dt, code_dt, NSim):
        ...
        while time < t_end:
            ...
            eject = self.resolve_ejection(gravity, bodies, cluster_lim)
            #if (eject):
             #   A0 += self.Acorr
              #  E0 += self.Ecorr
